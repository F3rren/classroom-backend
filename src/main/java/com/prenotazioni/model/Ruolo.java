...
public enum Ruolo {
    ...
    @JsonCreator
    public static Ruolo da(String valore) {
        if (valore == null) {
            return null;
        }
    ...
    public static class JpaConverter implements AttributeConverter<Ruolo, String> {

        @Override
        public String convertToDatabaseColumn(Ruolo ruolo) {
            return ruolo == null ? null : ruolo.getValore();
        }

        @Override
        public Ruolo convertToEntityAttribute(String valore) {
            return da(valore);
        }
    }
}
